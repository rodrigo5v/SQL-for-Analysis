...
3.Filter the accounts table to include the company name, website, and 
...
to find the unit price for standard paper for each order. Limit the 
...
division by zero in your formula. You will learn later in the course how to fully 
handle this issue. For now, you can just limit your calculations to the first 10 
orders, as we did in question #1, and you'll avoid that set of data that causes 
the problem.
*/
...
individuals who were contacted via the channel of organic 
or adwords.
*/
...
1.We can pull all of the rows that were excluded from the queries
in the previous two concepts with our new operator.

2.Use the accounts table to find the account name, primary poc, and
...
2.All companies whose names do not contain the string 'one' somewhere 
in the name.
...
3.When you use the BETWEEN operator in SQL, do the results include 
...
SELECT * FROM web_events
WHERE channel IN ('adwords','organic') AND
occurred_at BETWEEN '2016-01-01' AND '2017-01-01'
ORDER BY occurred_at;

/*
Questions using the OR operator
1.Find list of orders ids where either gloss_qty or poster_qty 
